...
$(document).ready(function() {
    ...
    reportPayslip = { printTable: function(search_data) { getpaginate(search_data, '#empPayslip', $('#paySlipSummary').val(), 'Only For Id, Name.'); } }
    reportPayslip.printTable(searchObj);

    reportEmpLeave = { printTable: function(search_data) { getpaginate(search_data, '#empLeaveSummary', $('#leaveSummary').val(), 'Only For Id, Name.'); } }
    ...
    $(".setZeroInpt").keyup(function() {
		...
		let esicEmpLyr = 0;
        let esicEmpLyee = 0;
        if($('#esicEmployee').val()==''){ esicEmpLyee = 0; } else { esicEmpLyee = parseFloat($('#esicEmployee').val()); }
		if($('#esicEmployer').val()==''){ esicEmpLyr = 0; } else { esicEmpLyr = parseFloat($('#esicEmployer').val()); }
...
/*********************************************************************/
    ...
    $(".arvChange").change(function()
	{
        ...
        if (actNbtn=='designation')
		{
            ...
            $.post(getResourceLoc, { id: $('#' + actNbtn).val(), action: actNbtn, branch: $('#branch').val() },
                function(htmlAmi) {
    ...
    $(".amiStl").click(function() {
		...
		else if(actNbtn=='AddNewDoc') {$('#lstDocDet').hide();$('#uplDocDetails').show();$('#edtDocDet').html('Add New Document');$('#AddNewDoc').hide();} 
		else if(actNbtn=='bckToDoclist') {$('#uplDocDetails').hide();$('#lstDocDet').show();$('#edtDocDet').html('List of documents');$('#AddNewDoc').show();} 
		else if(actNbtn=='compnyDetBtn') {$('#viewCmpnyDetails').hide();$('#edtCmpnyDetails').show();$('#edtCmpDet').html('Edit Company Details');} 
		else if(actNbtn=='proCompBck'){$('#edtCmpnyDetails').hide();$('#viewCmpnyDetails').show();$('#edtCmpDet').html(' Company Details');}
		else if(actNbtn=='backToAddEmp'){$('#fnlSuccess').hide();$('#frmRegister').show();}
		else if(actNbtn=='SalStpDetBtn'){ $('#viewSalarySetup').hide();$('#edtSalStpDetails').show();$('#edtSsl').html('Edit Salary Details');}
		else if(actNbtn=='proSalStpBck'){$('#edtSalStpDetails').hide();$('#viewSalarySetup').show();}
    });
...
$('#manage_document').submit(function(e) {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: 'json',
        beforeSend: function() { $('#saveDocDet').html('<i class="fe fe-settings bx-spin"></i> Please Wait'); },
        complete: function() { $('#saveDocDet').html('<i class="ti-save"></i> Save'); },
...
$('#edSalDetails').submit(function(e) {
    ...
    $.ajax({
        ...
        data: $(this).serialize(),
        dataType: 'json',
        beforeSend: function() { $('#saveSalStpkDet').html('<i class="fe fe-settings bx-spin"></i> Please Wait'); },
        complete: function() { $('#saveSalStpkDet').html('<i class="ti-save"></i> Save'); },
        success: function(htmlAmi)
		{
            toastMultiShow(htmlAmi.addClas, htmlAmi.msg, htmlAmi.icon);
        }
...
$('#edCmpnyDetails').submit(function(e) {
    ...
    $.ajax({
        ...
        data: $(this).serialize(),
        dataType: 'json',
        beforeSend: function() { $('#saveCompnyDet').html('<i class="fe fe-settings bx-spin"></i> Please Wait'); },
        complete: function() { $('#saveCompnyDet').html('<i class="ti-save"></i> Save'); },
        success: function(htmlAmi) {
            toastMultiShow(htmlAmi.addClas, htmlAmi.msg, htmlAmi.icon);
...
$('#add_employee_data').submit(function(e) {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: 'json',
        beforeSend: function() { $('#adEmployeeDetails').html('<i class="fe fe-settings bx-spin"></i> Please Wait'); },
        complete: function() { $('#adEmployeeDetails').html('<i class="ti-save"></i> Submit'); },
        success: function(htmlAmi) {
...
$('#employee_updata').submit(function(e) {
    ...
    $.ajax({
        ...
        data: $(this).serialize(),
        dataType: 'json',
        beforeSend: function() { $('#employeedataupdate').html('<i class="fe fe-settings bx-spin"></i> Please Wait'); },
        complete: function() { $('#employeedataupdate').html('<i class="ti-save"></i> Submit'); },
        success: function(htmlAmi) {
            toastMultiShow(htmlAmi.addClas, htmlAmi.msg, htmlAmi.icon);
            if (htmlAmi.addClas == 'tst_success') {
                setTimeout(function() { window.location.reload(1); }, 2000);
...
$('#resetYourPassword').submit(function(e) {
    ...
    $.ajax({
        ...
        data: $(this).serialize(),
        dataType: 'json',
        beforeSend: function() { $('#arvResetPassword').html('<i class="fe fe-settings bx-spin"></i> Please Wait'); },
        complete: function() { $('#arvResetPassword').html('<i class="ti-save"></i> Reset Password'); },
        success: function(htmlAmi) {
            toastMultiShow(htmlAmi.addClas, htmlAmi.msg, htmlAmi.icon);
            //if(htmlAmi.addClas=='tst_success'){setTimeout(function(){window.location.reload(1);},2000);}
        }
