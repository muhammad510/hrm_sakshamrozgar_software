...
$(document).ready(function () {
	...
	$(".empSelectR").change(function () {
		...
		if (actNbtn == "get_state") {
			...
			$.post(
				getResource + "software/setting/cityList",
				{ id: $("#" + actNbtn).val() },
				function (htmlAmi) {
			...
			$.post(
				getResource + "software/branch/manage",
				{ id: $("#" + actNbtn).val(), oprType: "department" },
				function (htmlAmi) {
					$("#district").html(htmlAmi);
...
$(function () {
	$(document).on("click", ".getAction", function () {
		...
		if (getData[0] == "miStatusView") {
			...
			let target = $("#base_url").val() + getData[1];
			$("#cnfChanges")
				.html('<i class="fe fe-setting bx-spin"></i> Please Wait')
			...
			$.post(
				...
				function (htmlAmi) {
					$("#cnfChanges")
						.html('Confirm <i class="bx bx-check-circle"></i>')
...
$("#manage_branch").submit(function (e) {
	...
	$.ajax({
		...
		dataType: "json",
		beforeSend: function () {
			$("#manage").html('<i class="fe fe-sun bx-spin"></i> Please Wait');
		},
		complete: function () {
			$("#manage").html('<i class="ti-save"></i> ' + manage);
		},
...
$("#designation_updata").submit(function (e) {
	...
	$.ajax({
		...
		dataType: "json",
		beforeSend: function () {
			$("#designationupddata").html(
				'<i class="bx bx-loader bx-spin"></i> Please Wait',
			);
		},
		complete: function () {
			$("#designationupddata").html('<i class="bx bx-save"></i> Submit');
		},
		success: function (htmlAmi) {
			toastMultiShow(htmlAmi.addClas, htmlAmi.msg, htmlAmi.icon);
			if (htmlAmi.addClas == "tSuccess") {
