$(document).ready(function() {
	...
	/*********************************************************************/
    $(".arvSel").unbind("change").change(function()
	{
       let isCreate=$("#basicUpdtID").val();
	   if(isCreate=="")
	   {
			let actNbtn = $(this).attr('id');let getTrgtId = $(this).attr('data-id');
			$('#designation').css('color', '#099b7e !important').html('<option>Please Wait.....</option>');
			$.post(getTrgtId,{ brID:$('#'+actNbtn).val()},function(htmlAmi) 
			{
				$('#designation').html(htmlAmi);$('#designation').css('color', 'rgb(62, 62, 62) !important');
			});
	   }
	});
/*********************************************************************/
                ...
                if (htmlAmi.addClas != 'tst_danger') {
                    if (actnstr[0] == 'viewDetails') {
                      ...
                      /*  $('#vGrsAmt').html(htmlAmi.gross_sal_amt);*/
                        ...
                        $('#vPfPayEmplyr').html(htmlAmi.pfAmt);
						$('#vAdvncPay').html(htmlAmi.advance_amt);
                        $('#vTdsPay').html(htmlAmi.tdsAmt);
						
						
						$('#esicEmp').html(htmlAmi.esicEmpAmt);
						$('#esicEmpLyr').html(htmlAmi.esicEmpLyrAmt);
					...
					else if (actnstr[0] == 'editDetails')
					{
						...
						$('#esicEmployee').val(htmlAmi.esic_employee);
						$('#esicEmployer').val(htmlAmi.esic_employer);
						$('#esicEmpAmt').val(htmlAmi.esicEmpLyrAmt);
						$('#esicEmpPayAmt').val(htmlAmi.esicEmpAmt);
