	...
	$(".arvSel")
		...
		.change(function () {
			...
			if (isCreate == "") {
				...
				let placementId = $(this).data("placement-id");
				let id = $(this).val();

               let branch_id=$('#brOffice').val();
               
				$("#" + placementId)
					.css("color", "#099b7e !important")
					.html("<option>Please Wait.....</option>");
				$.post(getTrgtId, { brID: branch_id,id:id }, function (htmlAmi) {
					$("#" + placementId).html(htmlAmi);
					$("#" + placementId).css("color", "rgb(62, 62, 62) !important");
				});
			}
		});
	/*********************************************************************/
...
$(document).ready(function () {
	...
	reportBranch = {
		printTable: function (search_data) {
			getpaginate(search_data, "#branch_det", target, "Only For Id, Name.");
		},
	};
	...
	$("#branch_det,#statusChange").on("click", ".arvAction", function () {
		...
		if (actnstr[0] == "viewDetails" || actnstr[0] == "editDetails") {
			...
			$.post(
				target,
				{ oprType: actnstr[0], id: actnstr[2] },
				function (htmlAmi) {
			...
			let target = $("#base_url").val() + actnstr[1];
			$("#cnfChanges")
				.html('<i class="fe fe-settings bx-spin"></i> Please Wait')
			...
			$.post(
				...
				function (htmlAmi) {
					$("#cnfChanges")
						.html('Confirm <i class="si si-check"></i>')
						.removeClass("btn-outline-primary")
						.addClass("btn-outline-secondary");
					$("#arvs--" + actnstr[2])
						.addClass(htmlAmi.btnAdCls)
						.removeClass(htmlAmi.btnRmvCls)
						.html(htmlAmi.btnTxt);
					$(".actnData").html(htmlAmi.msg);
