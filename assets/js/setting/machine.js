var reportBranch = '';
$(document).ready(function() {
    let searchObj = {};
    var target = $('#target').val();
    reportBranch = { printTable: function(search_data) { getpaginate(search_data, '#machine_det', target, 'Only For Id, Name.'); } }
    reportBranch.printTable(searchObj);
    $(".machineManage").click(function() {
        let actNbtn = $(this).attr('id');
        if (actNbtn == 'machineSrch' || actNbtn == 'bkSrch') {
            $("i", "#machineSrch").toggleClass("ti-search ti-minus");
            $("#searchMachineDetails").toggle();
        } else if (actNbtn == 'addNewMachineDetails') {
            //$('#gtBrCode').val($('#newBranchID').text());
            $("#searchMachineDetails,#machineSrch,#amiResult").hide();
            $("i", '#addNewMachineDetails').toggleClass("ti-plus ti-minus");
            $("#amiMachineAllChanges").show();
            $('#addNewMachineDetails').attr('id', 'backToListShow');
            $('#bxTitleNm').html('Add Machine');
            $('#oprType').val($(this).attr('data-id'));
            $('#manage').html('<i class="ti-save"></i> Submit');
        } else if (actNbtn == 'backToListShow' || actNbtn == "bkSrch_01") {
            $("i", '#backToListShow').toggleClass("ti-plus ti-minus");
            $('#backToListShow').attr('id', 'addNewMachineDetails');
            $('#bxTitleNm').html('Machine Manage');
            $("#amiMachineAllChanges").hide();
            $("#amiResult,#machineSrch").show();
        }
    });
    $(".empSelectR").change(function() {
        var actNbtn = $(this).attr('id');
        var getResource = $('#base_url').val();
        if (actNbtn == 'get_state') {
            $('#district').html('<option>Please Wait.....</option>');
            $('#district').css('color', '#099b7e');
            $.post(getResource + "software/setting/cityList", { id: $('#' + actNbtn).val() }, function(htmlAmi) {
                $('#district').html(htmlAmi);
                $('#district').css('color', 'rgb(13, 62, 115)')
            });
        }
    });
});
$(function() {
    $(document).on("click", '.getAction', function() {
        let actNbtn = $(this).attr('data-id');
        let getData = actNbtn.split('===');
        if (getData[0] == 'miStatusView') {
            let target = $('#base_url').val() + getData[1];
            $.post(target, { oprType: getData[0], id: getData[2] }, function(htmlAmi) {
                $('.actnData').html(htmlAmi.msg);
                $('#cnfChanges').attr('data-id', htmlAmi.action);
            }, 'json');
        } else if (getData[0] == 'miStatusChange') {
            let target = $('#base_url').val() + getData[1];
            $('#cnfChanges').html('<i class="fe fe-settings bx-spin"></i> Please Wait').removeClass('btn-outline-secondary').addClass('btn-outline-primary');
            $.post(target, { oprType: getData[0], id: getData[2] }, function(htmlAmi) {
                $('#cnfChanges').html('Confirm <i class="si si-check"></i>').removeClass('btn-outline-primary').addClass('btn-outline-secondary');
                $("#arvs--" + getData[2]).addClass(htmlAmi.btnAdCls).removeClass(htmlAmi.btnRmvCls).html(htmlAmi.btnTxt);
                $('.actnData').html(htmlAmi.msg);
                $('#statusChange').delay(3000).modal('hide');
            }, 'json');

        } else if (getData[0] == 'miMachineView' || getData[0] == "miBrEdt") {
            let acBtn = '';
            if (getData[0] == "miBrEdt") {
                acBtn = '#edM0' + getData[2];
                $("#manage").show();
            } else {
                acBtn = '#vwM0' + getData[2];
                $("#manage").hide();
            }
            let target = $('#base_url').val() + getData[1];
            $(acBtn).html('<i class="fe fe-sun bx-spin"></i>');
            $.post(target, { oprType: getData[0], id: getData[2] }, function(htmlAmi) {
                //$('#bxTitleNm').html(htmlAmi);
                $(acBtn).html('<i class="ti-eye"></i>');
                if (htmlAmi.addClas == "tst_success") {
                    $('#oprType').val(getData[0]);
                    $('#target').val(htmlAmi.id);
                    $('#gtMchneCode').val(htmlAmi.code);
                    $('#machineBrand').val(htmlAmi.machine_brand);
                    $('#machineModelNo').val(htmlAmi.machine_model_no);
                    $("#machineDeviceNo").val(htmlAmi.device_id);
                    $("#machineIpAddr").val(htmlAmi.machine_ip_address);
                    $("#machinePort").val(htmlAmi.machine_port_no);
                    $("#machineLocation").val(htmlAmi.machine_location);
                    $('#get_state').html(htmlAmi.getState);
                    $('#district').html(htmlAmi.getDistrict);
                    $("#zipCode").val(htmlAmi.zipcode);
                    $('#con_mode').html(htmlAmi.connection_mode);
                    $('#machineSno').val(htmlAmi.serial_no);
                    $("#machineRemarks").val(htmlAmi.remark);
                    $("#searchMachineDetails,#machineSrch,#amiResult").hide();
                    $("i", '#addNewMachineDetails').toggleClass("ti-plus ti-minus");
                    $("#amiMachineAllChanges").show();
                    $('#addNewMachineDetails').attr('id', 'backToListShow');
                    if (getData[0] == "miMachineView") {
                        $('#manage').hide();
                        $('#bxTitleNm').html('View Machine Details');
                    } else if (getData[0] == "miBrEdt") {
                        $('#manage').html('<i class="ti-save"></i> Update').show();
                        $('#bxTitleNm').html('Edit Machine Details');
                    }
                } else { toastMultiShow(htmlAmi.addClas, htmlAmi.msg, htmlAmi.icon); }
            }, 'json');

        }
    });
});

$('#manage_branch').submit(function(e) {
    let target = $('#base_url').val() + 'software/machine/manage/';
    let action = $('#oprType').val();
    let manage = '';
    if (action == 'addNew') { manage = "Submit"; } else if (action == 'miBrEdt') {
        manage = "Update";
        $('#oprType').val('updateBrDet');
    } else if (action == 'updateBrDet') {
        manage = "Update";
        $('#oprType').val('updateBrDet');
    } else { manage = "Arv Action"; }
    e.preventDefault();
    $.ajax({
        url: target,
        type: "POST",
        data: $(this).serialize(),
        dataType: 'json',
        beforeSend: function() { $('#manage').html('<i class="fe fe-settings bx-spin"></i> Please Wait'); },
        complete: function() { $('#manage').html('<i class="ti-save"></i> ' + manage); },
        success: function(htmlAmi) {
            toastMultiShow(htmlAmi.addClas, htmlAmi.msg, htmlAmi.icon);
            if (htmlAmi.addClas == 'tst_success') { setTimeout(function() { window.location.reload(1); }, 2000); }
        }
    });

});