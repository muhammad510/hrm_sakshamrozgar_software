...
$(".miUpload").on("click", function() {
	...
	let actNbtn=$(this).attr('id');
	var myArray = {"darkFavicon":"dark_fabcon","lightFavicon":"light_fabcon", "darkLogo":"dark_logo", "lightLogo":"light_logo"};
	var lgImg = document.getElementById(myArray[actNbtn]).files[0];	
	if(!isCheck(myArray[actNbtn])){let errMSG=["Please input logo image"];toastMultiShow('tst_danger',errMSG,'<i class="fe fe-settings bx-spin"></i>');}
	else{		
			let base_url=$('#base_url').val();
			f = new FormData();
			f.append('file', lgImg);
