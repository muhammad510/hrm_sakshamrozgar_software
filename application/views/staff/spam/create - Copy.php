<div class="inner-body">
				...
				.miHeader{padding: 20px 15px 15px 15px;border-bottom: 1px solid #cccece;margin: 0px -10px 20px -10px;border-top-left-radius: 5px;border-top-right-radius: 5px;}
						...
						<div class="row row-sm">
							<div class="cardTtl">		
								...
								<div class="col-md-12 col-lg-12">
									<div class="row row-sm" id="amiResult" style="background-color:#f0f0f0;margin-bottom:15px;padding:10px 0px 10px 0px;border: 1px solid #e3e2e3;">
											Data will here show
								    </div>
									<div id="paginaTation" style="padding:10px; border:1px solid #e3e3ea;margin: 10px -10px 10px -10px;font-weight: bold;">
										Number of page change
									</div>
									
									<div id="amiPage" style="margin: 10px -10px 10px -10px;">
									
									
									Here Page willl show
...
$(function()
{
	$(document).ready(function()
	{
		$(".amiPg").click(function()
		{
			...
			let currentCls='';
			// = $("#"+cID).hasClass("amiNext");
			//if(cID.substring(4))
			
			
			//$('#paginaTation').html(actv.substring(3)+'==='+cID+" Length "+cID.length);
			let liCnt=$('#amiPagination').children("li").length;
			let next=liCnt-2;
			let amiPrv=parseInt(cID)-parseInt(1);
			let amiNxt=parseInt(cID)+parseInt(1);
			//$('#paginaTation').html(actv+"==="+dataId+cID+" current class "+currentCls);
 ...
 /*  $('#amiResult').html('Mere me change chahiy tha na ki dikkat ho');
   
   	$.post(getResource+"merahaat/shipping_add",
			{id:$(this).attr('data-id'),getUserN:getUserN},
			function(html){
				$('#seOthrAdresTitle').css('color','#016a01')
				$("#seOthrAdresTitle").html('<i class="fa fa-map-pin"></i> Add New Address');
				$(".seOthrAdresMsgOnPopUp").html(html); 
			}   
		);*/
