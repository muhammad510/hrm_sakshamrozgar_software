	<div class="inner-body">
		...
		<?php //print_r($employee);?>
                ...
                <div class="card custom-card">
                    <div class="card-header"><h3 class="main-content-label">My Account</h3></div>
                                                                    ...
                                                                    <table class="table border text-md-nowrap text-nowrap" id="empDocTble">
                                                                        ...
                                                                        <tbody>
                                                                           ...
                                                                           <tr class="border-bottom miCenter" id="miDcSrl-<?php echo $empD->id;?>">
                                                                              ...
                                                                              <!--  <td><?php //echo($empD->modified_date)?date('d-m-Y',strtotime($empD->modified_date)):'<span class="naSpc">N/A</span>';?></td>-->
                                                                                <td>
                                                                          <a href="javascript:void(0);" data-id="viewDetails===<?php echo $empD->id;?>===<?php echo $docTypArr[$empD->doc_type];?>" title="View" class="btn btn-outline-primary btn-sm viewEmpDocDetails">
                                                                            ...
                                                                            <?php }}else {?>
                                                              <tr><td colspan="6"><div class="midocImg"><div>No document available in here</div> <img src="<?php echo base_url('uploads/addnewitem.svg');?>"></div></td></tr>
                                                                            <?php }?>
                                                                        </tbody>
                                                                    </table>
                                                              </div>	
    ...
    <div class="modal-content">
        <div class="modal-body">
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="float:right;"><i class="si si-close"></i></button>
                ...
                <style>
					input[type=file]::file-selector-button {margin-right: 20px !important;border: none;background:#dbe0ea  !important;padding: 10px 20px;border-radius:5px;color:#0D3E73;cursor: pointer;transition: background .2s ease-in-out;}
					input[type=file]::file-selector-button:hover {background: #D9E0EE;}
					#uploadMemberIMg{padding:4rem;}
					#proImgErr{ color:#c84c02;text-align: right; display:none;font-size: 12px;}
					</style>                   
				<div class="row row-sm">	
 ...
 $(document).on("click", '.viewEmpDocDetails', function()
{
   		...
   		let actNbtn=$(this).attr('data-id');let getData=actNbtn.split('===');	
		if(getData[0]=='viewDetails')
		{let imgLocDetails=$('#empDc--'+getData[1]).attr('src');$('#empModalImgShw').show();$('#empImgShwDet').attr('src',imgLocDetails);$('#miCaption').html(getData[2]);}
		else if(getData[0]=='miClose'){$('#empModalImgShw').hide();$('#empImgShwDet').attr('src','');$('#miCaption').html('');}
		else if(getData[0]=='trashDocDetails')
		{
			let cnfDelAction='cnfDeleteDoc==='+getData[1]+'==='+getData[2];
			$('.actnData').html('<i class="si si-power"></i> Are you sure want to delete '+getData[3]+' document details');$('#cnfChanges').attr('data-id',cnfDelAction);
			}
			else if(getData[0]=='cnfDeleteDoc')
			{
				$('#cnfChanges').html('<i class="fe fe-settings bx-spin"></i> Please Wait');let getResourceLoc=$('#base_url').val()+getData[1];
				$.post(getResourceLoc,{id:getData[2],action:getData[0]},function(htmlAmi)
					  {
						  if(htmlAmi.addClas=='success')
						  {							  	
								$.each(htmlAmi.srCnt,function(index,value){sum=1+index;$("#smDocCnt-"+value).html(sum+'.');})
								$('.actnData').css('color','#059B1B').html(htmlAmi.msg);$("#miDcSrl-"+getData[2]).remove();$('#trashEmpDetails').delay(3000).modal('hide');
								}
						  else{$('.actnData').css('color','#ca0000').html(htmlAmi.msg);}
						},'json');
				}
...
{		
$(".memberImgUploadActn").click(function(){var imgFile=$('#file').val();if(imgFile==""){$('#proImgErr').html('Please provide employee image').fadeIn().delay(2000).fadeOut();}else{$('#empProImgBtn').html('<i class="fe fe-settings bx-spin"></i> Please Wait');
var name = document.getElementById("file").files[0].name;var form_data = new FormData();var ext = name.split('.').pop().toLowerCase();if(jQuery.inArray(ext, ['gif','png','jpg','jpeg']) == -1){$('#proImgErr').html('Please provide valid image format').fadeIn().delay(2000).fadeOut();}var oFReader = new FileReader();oFReader.readAsDataURL(document.getElementById("file").files[0]);var f = document.getElementById("file").files[0];var fsize = f.size||f.fileSize;if(fsize > 2000000){$('#proImgErr').html('Image File Size is very big').fadeIn().delay(2000).fadeOut();}else{form_data.append("file", document.getElementById('file').files[0]);$.ajax({url:$('#empProImgBtn').attr('data-id'),method:"POST",data: form_data,contentType: false,cache: false,processData: false,beforeSend:function(){$('#Update').html('Wait...');},   success:function(data){$('#empProImgBtn').html('<i class="si si-picture"></i> Update');var strng=data.split("====");if(strng[0]=='1'){$('#empProfilePic').attr('src',base_url+"uploads/employee/"+strng[1]);$('#proImgErr').html('successfully upload image').css('color','green').fadeIn().delay(2000).fadeOut();setTimeout(function(){window.location.reload(1);},2500);}else{$('#proImgErr').html('Some Error Occur Please Re-Update').fadeIn().delay(2000).fadeOut();}}});}}
});
});	
...
.select2-container--default .select2-selection--single .select2-selection__rendered{color: #1c2e6f !important;}	
    ...
    .icnClr i{ 
        padding:5px;background-color: #4D43B0;border-radius:5px;margin-right: 5px;color:white;
    }
...
.hedings a i{padding:8px;background-color:#fff;border-radius:20px;color:#047713;margin-right:15px;}  .hedings a{ float:right;}#imgUpload{ margin-top: -25px;}
