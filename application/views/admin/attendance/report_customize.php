...
.miBgs{width:3.5rem;padding:5px 0px 5px 0px;}.avMi{width: 3rem;height: 3rem;line-height:3rem;position:relative;text-align:center;display:inline-block;color:#fff;font-weight:600;vertical-align:bottom;font-size:1.3rem;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;}.bdrBottom{ border-bottom:1px solid #e6e6e6;margin:0px -25px 30px -25px;}.ami_tHeader > tr > th {color: #FFFFFF !important;border: 1px solid #088 !important;border-top-width: 1px;border-bottom-width: 1px;border-bottom-style: solid;border-bottom-color: rgb(0, 136, 136);padding: 12px 0px 12px 5px !important;}.ami_tHeader{ background-color:#088 }.miBottom{margin-bottom: 75px;}.dark-theme .bdrBottom{ border-bottom:1px solid #2d2d48;}.dark-theme .ami_tHeader{ background-color:#025959}.dark-theme .ami_tHeader > tr > th{border: 1px solid #027373 !important;}.mSrchPnl{ margin:-1px 0px 0px 0px;position: absolute;width: 290px;z-index: 1;cursor: pointer; display:none;}
.mSrchPnl ul{ list-style:none;background-color:#fff; border: 1px solid #d9d9d9;border-bottom-left-radius: 6px;border-bottom-right-radius: 6px;-webkit-box-shadow: 0px 3px 5px 0px rgba(145,145,145,1);-moz-box-shadow: 0px 3px 5px 0px rgba(145,145,145,1);
box-shadow: 0px 3px 5px 0px rgba(145,145,145,1);max-height: 230px;overflow-x: hidden;overflow-y: scroll;scrollbar-width: thin;scrollbar-color: #24839f #fff;}
.mSrchPnl ul li{ padding:8px 0px 8px 8px; border-bottom:1px solid #d9d9d9;margin-left: -31px; color: #757575;font-weight: 600;}.mSrchPnl ul li:last-child{ border-bottom:0px solid #000;} .mSrchPnl ul li span{color: #c65d00;padding-left: 5px;}
...
.customTitle{ padding-bottom:50px;}
.thCl {
  padding: 0px 25px 0px 5px;
  margin: 0px;text-align: center;
}
.inOutDet div{ float:left; text-align: center;width: 50%;}
.inOutDet div:first-child{ border-right:1px solid #bdb0b0;}
.inOutDet{ width: 125px;text-align: center;}
.ami_tHeader > tr:last-child > th {padding: 3px 10px 3px 8px !important;}
.ami_tHeader > tr:first-child > th {padding: 3px 10px 3px 8px !important;}
.myNaming{margin: 0px 0px 15px 0px;}
.mbrPad{padding:20px 0px 18px 15px !important;}
.mbrPad:first-child{margin-top: -10px;}
.mbrPad:last-child{margin-bottom: -8px;}

.mbrPad:hover{color: #ffffff !important;background-color: var(--export-bg-color);border-color: var(--export-bg-color);}
</style>
        ...
        <div class="table-responsive">
          ...
          <table class="table text-nowrap text-md-nowrap table-bordered table-hover mg-b-0 " id="getReportMiDetails">
            <thead class="ami_tHeader" id="setMonthWise">
                <tr>    
                    <th rowspan="2"><div class="thCl myNaming">S.No.</div></th>
                    <th rowspan="2"><div class="thCl myNaming">EMP. ID</div></th>
                    <th rowspan="2"><div class="thCl myNaming">EMPLOYEE NAME</div></th>
                    <?php for ($i = 1; $i <= $dayInMonth; $i++) : ?><th><div class="thCl"><?php echo date((($i<10)?('0'.$i):$i).'-m-Y'); ?></div></th><?php endfor; ?>
                    <th rowspan="2"><div class="thCl myNaming">T. Present</div></th>
                    <th rowspan="2"><div class="thCl myNaming">T. Absent</div></th>
                    <th rowspan="2"><div class="thCl myNaming">T. Leave</div></th>
                </tr>
                 <tr>
                    <?php for ($i = 1; $i <= $dayInMonth; $i++) : ?><th>
                    		<div class="thCl">
									
                                    <div class="inOutDet"><div>In</div><div>Out</div></div>
                            </div>
                     </th><?php endfor; ?>
                </tr>
            </thead>
...
$(function() 
 {
	$(document).unbind("click").on("click", '.getAction', function() 
	{
		...
		{
			let targetUrl=$(this).attr('data-id');$('#getReportMiDetails tbody').empty();
			$.post(targetUrl,{fromDate:$('#fromDate').val(),toDate:$('#toDate').val()},function(htmlAmi)
			{
				$('#setMonthWise').html(htmlAmi);					
				setTimeout(function(){$('#getReportMiDetails tbody td,thead th').each(function() {if ($(this).html().trim() === '') {$(this).remove();}});},4000);
				});
			}
		else if((actNbtn=='excelExport')||(actNbtn=='pdfExport'))
		{
			...
			$.post($('#base_url').val()+target[1],{action:target[0],fromDate:$('#fromDate').val(),toDate:$('#toDate').val(),activePage:activePage},function(htmlAmi)
			{
				if(htmlAmi.addClas=='tst_success')
				{
					if(actNbtn=='excelExport'){window.location.href=htmlAmi.downlink;}
					if(actNbtn=='pdfExport'){window.open(htmlAmi.downlink, '_blank').focus();}
