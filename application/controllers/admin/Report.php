...
{  
	...
	if(($post['action']=='excelExport')||($post['action']=='pdfExport'))
	{
			...
			if($getData)
			{
				...
				else{$return=array('addClas'=>'tst_warning','msg'=>array(' Unfortunately, there are no records available.'),'icon'=>'<i class="fe fe-settings bx-spin"></i>');}
		}
		else{$return=array('addClas'=>'tst_warning','msg'=>array(' Unfortunately, there are no records available.'),'icon'=>'<i class="fe fe-settings bx-spin"></i>');}
...
public function download($where)
{
	...
	if($where->actn=='excelExport')
	{
		...
		//echo $this->db->last_query();//print_r($headers);print_r($getRecord);
        ...
        $column = 'A';$row = 1;
		foreach ($headers as $index => $header) {$objPHPExcel->setActiveSheetIndex(0)->setCellValue($column . $row, $header);$column++;}
        $row = 2;
	...
	else {$return=array('addClas'=>'tst_success','msg'=>array('Oops! It seems you’ve selected an invalid option.','Please check your choice and try again.'),'icon'=>'<i class="fe fe-settings bx-spin"></i>'); echo json_encode($return);}	
