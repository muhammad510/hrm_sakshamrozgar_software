...
public function getCompanyDetails()
{
	...
	{
		if($post['branch']!='9999'){$return=$this->common->get_data('salary_master',array('status'=>'1','branch_id'=>$post['branch'],'desig_id'=>$post['id']),'id,net_payble_amt');}
	...
	{				
		...
		if($post['slID']!='9999'){$getSalAmt=$this->common->getRowData('salary_master','id',$post['slID']);}else{$getSalAmt->net_payble_amt='50000';}
...
/*****************************************@mit Singh Start 01-03-24 ****************************************************/
        ...
        public function create()
        {
           $result = $this->db->select('id,employee_id')->from('staff')->order_by('id', 'DESC')->get()->row();		
			...
			if(($this->logCat!='5')&&($this->logCat!='4')){$fieldDet='status';$fieldID='1';}else{$fieldDet='id';$fieldID=$this->logBr;}
        ...
        public function manages($where=NULL)
		{
           if($where)
		   {
				...
				if($where->action=='editDetails')
				{
					...
					if($employee->user_type=='1'){$data['salID']='9999';}else{$data['salID']=$employee->salary_id;}		
...
/***********************************************************************************************/
		public function grid()
	   {
			...
			$data=array('title'=>'Manage Employee Details',
	...
	if($post['actnType']=='department')
	{
		  ...
		  if($getDetails['department'])
		  {
		      ...
		      if($department)
			  {
				...
				else{$result='<option value="">Choose Department</option>';}				
		  	}
			else{$result='<option value="">Choose Department</option>';}
		}
...
public function import($action=NULL)
{
		...
		else if($action=='readDoc')
		{
			...
			{
				$getFileSize=filesize($getFile);$lastId='';
				if($getFileSize > 2)
				{
					 $file=fopen($getFile, "r");
					 ...
					 {
						$empID+=1;
						$gender=($emapData[2]=='Male')?'1':(($emapData[2]=='Female')?'2':'3');$dateOfBirth=date('Y-m-d',strtotime($emapData[3]));
						$resultArr=array('employee_id'=>$empID,'biometric_id'=>$empID,'user_type'=>'4','name'=>$emapData[1],'gender'=>$gender,'dob'=>$dateOfBirth,'contact_no'=>$emapData[4],'email'=>$emapData[5],'address'=>$emapData[6],
										 'password'=>md5($empID),'show_password'=>$empID);
					 	...
					 	if($lastId)
						{
							...
							$msg=array('Thank You! you have successfully import employee details.');$data=array('addClas'=>'tst_success','msg'=>$msg,'icon'=>'<i class="ti-check-box"></i>');
							}
						else {$msg=array('Oops it seems went wrong while importing details.');$data=array('addClas'=>'tst_warning','msg'=>$msg,'icon'=>'<i class="fe fe-settings bx-spin"></i>');}
					 }
					 else{$msg=array('Oops it seems there is no data in this file.');$data=array('addClas'=>'tst_danger','msg'=>$msg,'icon'=>'<i class="fe fe-settings bx-spin"></i>');}
				}
				else{$msg=array('Oops it seems there is not file exist.');$data=array('addClas'=>'tst_danger','msg'=>$msg,'icon'=>'<i class="fe fe-settings bx-spin"></i>');}
					sleep(1);
					echo json_encode($data);
			}	
			else
...
public function performance($actn=NULL)
{
	if($actn=='viewList')
	{
		...
		{
			$data=array('title'=>'Employee Performance Details','breadcrums'=>'Employee Performance Details',
						'target'=>"admin/employee/performance/viewList",'layout'=>"admin/employee/performance.php",'bckUrl'=>base_url("staff/dashboard"));
			$this->load->view('base', $data);
			}
	}
...
public function generate($actn=NULL)
{
		  ...
		  if($post)
		  {
			 ...
			 if($isExistEmp)
			 {
				...
				$return=array(
							  ...
							  'msg'=>array("Thank You! You have successfully submitted your remakrs."),'icon'=>'<i class="ti-check-box"></i>'
							  );
				}
				else{$return=array('addClas'=>'tst_danger','msg'=>array("Details not available for this ID #".$post),'icon'=>'<i class="fe fe-sun bx-spin"></i>');}
			}
			else{$return=array('addClas'=>'tst_danger','msg'=>array("Please input employee id to get details."),'icon'=>'<i class="fe fe-sun bx-spin"></i>');}
			echo json_encode($return);
		}
	else if($actn=='offer_letter')
	{
			$data=array('title'=>'Employee Offer Letter','breadcrums'=>'Offer Letter','target'=>"searchEmployee===admin/employee/generate/forJoining===find_emp_id",
						'layout'=>"admin/employee/offer_letter.php",'bckUrl'=>base_url("staff/dashboard"));
			$this->load->view('base', $data);
		}
	else if($actn=='forJoining')
	{
		 $post=$this->input->post('id');
		  if($post)
		  {
			 ...
			 if($isExistEmp)
			 {
				...
				$return=array('addClas'=>'tst_success','empCode'=>$isExistEmp->employee_id,'empName'=>$fullName,'empContact'=>$isExistEmp->contact_no,'designation'=>$isExistEmp->designation_name,
							  'department'=>$isExistEmp->department_name,'address'=>$isExistEmp->address,'cityPin'=>$cityPin,'zipCode'=>$isExistEmp->zipcode,'created_at'=>date('d M,Y',strtotime($isExistEmp->created_at)),
							  'basic_sal'=>$basic,'hraAmt'=>$hraAmt,'taAmt'=>$taAmt,'daAmt'=>$daAmt,'paAmt'=>$paAmt,
							  'bonus'=>$bouns,'medical'=>$medical,'insentive'=>$insentive,'otherInc'=>$otherInc,'pfAmt'=>$pfAmt,
							  'tdsAmt'=>$tdsAmt,'insurance'=>$insurance,'othrDeduction'=>$othrDeduction,'grossAmt'=>$grossAmt,'advance'=>$advance,
							  'msg'=>array("Thank You! You have successfully submitted your remakrs."),'icon'=>'<i class="ti-check-box"></i>');
				}
				else{$return=array('addClas'=>'tst_danger','msg'=>array("Details not available for this ID #".$post),'icon'=>'<i class="fe fe-sun bx-spin"></i>');}
			}
		...
		{
				$data=array('title'=>'Employee Resignation Letter','breadcrums'=>'Resignation Letter','layout'=>"admin/employee/regination_letter.php",
							'target'=>"searchEmployee===admin/employee/generate/forRegination===find_emp_id",'bckUrl'=>base_url("staff/dashboard"));
				$this->load->view('base', $data);
			}
		...
		else
		{
			...
			$data=array('title'=>'Employee Form 16 generate','breadcrums'=>'Employee Form 16 generate','getCompanySt'=>$getCompanySt,
						'target'=>"searchEmployee===admin/employee/generate/viewList===find_emp_id",'layout'=>"admin/employee/form16_generate.php",'bckUrl'=>base_url("staff/dashboard"));
			$this->load->view('base', $data);
			}
	}
...
public function profile_image($id)
{
	...
	{		$image['image_upload']=array('upload_data' => $this->upload->data()); //Image Upload
		...
		//	$config['new_image']  = 'uploads/user/thumb_image/'.$image_filename;
			...
			if($result){$resltMsg='1===='.$image_filename;}else{$resltMsg='2';}
				echo $resltMsg;
		}
			else
			{
...
public function notify()
{
	...
	{
		$nofyClr="#019927";$nofyHeadClr="#004411";
		...
		{
			$nofyClr="#AE7200";$nofyHeadClr="#603F00";
			...
			{
				$nofyClr="#1C5AD0";$nofyHeadClr="#002468";
				...
				{
					$nofyClr="#E13F83";$nofyHeadClr="#990040";
...
public function send_mail()
{
	...
	if($post['id']!="")
	{
		...
		{	
			$where=array('name'=>$isEmployee->employee_name,
			...
			$notfyClr="#00668E";$notfyHeadClr="#004966";
			...
			$headers  .= 'From: '.config_item('company_name').'. <support@ashutoshautofin.in>';
		...
		{	
			  ...
			  $where=array('empCode'=>$isEmployee->employee_id,
		...
		else{$notfyClr="#880026";$notfyHeadClr="#880026";$msg=" Please be patient, updates are in progress for this section.<br>We'll share the latest information with you shortly. Thank you!";}	
		 $return=array('result'=>'tSuccess','msg'=>$msg,'notfyClr'=>$notfyClr,'notfyHeadClr'=>$notfyHeadClr,'sendMail'=>'alreadyMail');			
		}
		else{$return=array('result'=>'tDanger','msg'=>"I can't find any relevant data for the selected option.",'notfyClr'=>"#ff3000",'notfyHeadClr'=>"#ae2100");}
		echo json_encode($return);	
    }

	public function testMode()
	{
		$data=array('title'=>'Email Template Test Mode',
	   ...
	   {
		$data=array('title'=>'Active Employee',
	  ...
	  public function active_employees()
	  {
	   ...
	   {
		$data=array('title'=>'Resigned Employee',
		'btnName'=>'Show List',
		...
		{
		 $data=array('title'=>'Terminated Employee',
