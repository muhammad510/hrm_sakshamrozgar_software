...
class Attendance extends CI_Controller
{
	...
	public function daily($actn = NULL)
	{
		if ($actn == 'viewList') {
			...
			foreach ($record as $row) {
				...
				if ($getAttnStatus->staff_attendance_type_id == 1) {
					$attendance_status = '<span class="badge mibdge" style="background-color:green;">Present</span>&emsp;';
				} elseif ($getAttnStatus->staff_attendance_type_id == 2) {
					$attendance_status = '<span class="badge mibdge" style="background-color:#CC7000;">Late</span>&emsp;';
				} elseif ($getAttnStatus->staff_attendance_type_id == 3) {
					$attendance_status = '<span class="badge mibdge" style="background-color:#F16D75;">Absent</span>&emsp;';
				} elseif ($getAttnStatus->staff_attendance_type_id == 4) {
					$attendance_status = '<span class="badge mibdge" style="background-color:#0073d9;">Holiday</span>&emsp;';
				} elseif ($getAttnStatus->staff_attendance_type_id == 5) {
					$attendance_status = '<span class="badge mibdge" style="background-color:#D9AD03;">Half Day</span>&emsp;';
				} elseif ($getAttnStatus->staff_attendance_type_id == 6) {
					$attendance_status = '<span class="badge mibdge" style="background-color:#009EA6;">On Leave</span>&emsp;';
				} else {
				...
				if ($getAttnStatus->log_in_time) {
					$log_in_time = date('h:i:s a', strtotime($getAttnStatus->log_in_time));
				} else {
					$log_in_time = '';
				}
				if ($getAttnStatus->log_out_time) {
					$log_out_time = date('h:i:s a', strtotime($getAttnStatus->log_out_time));
				} else {
					$log_out_time = '';
				}
				...
				if ($getAttnStatus->log_out_time) {
					...
					$workingHrs = (($getAttnStatus->staff_attendance_type_id == '6') || ($getAttnStatus->staff_attendance_type_id == '3')) ? '' : '<span class="wrKng">Still working...</span>';
				}
				...
				$return['data'][] = array(
					...
					$log_out_time,
					$workingHrs,
					/*$getAttnStatus->staff_attendance_type_id,*/
					$attendance_status,
					$view
				);
			}
	...
	public function update_attendance_status()
	{
		...
		if ($employee_data['attendance_date']) {
			...
			$logOutTime = date('H:i:s', strtotime($employee_data['shift_end']));
			$mrkAttendance = NULL;
		}
		...
		if ($employee_data) {
			...
			$details = array(
				'id' => $employee_data['id'],
				'empNmWithID' => $employee_data['name'] . ' (' . $employee_data['employee_id'] . ')',
				'attendance_status' => $employee_data['attendance_status'],
				'loginDt' => $loginDateTime,
				'logOutDt' => $logOutDateTime,
				'attStatus' => $mrkAttendance
			);
			sleep(1);
	...
	public function update_attendance_status_data()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if (empty($att)) {
				$attendance = array(
					'employee_id' => $post['id'],
					'staff_attendance_type_id' => $post['attendance_status'],
					'attendance_date' => $attendance_date,
					'log_in_time' => $log_in,
					'in_location' => $post['getPosition'],
	...
	public function reportList($getActn = NULL)
	{
		if ($getActn == 'viewList') {
			...
			if (($post_data['monthSearch'] != '') && ($post_data['yearSearch'] != '')) {
				...
				$searchDateMonth = date('Y-m-'); //$dayInMonth='29';
			}
				...
				for ($mi = 1; $mi <= 31; $mi++):
					if ($mi <= $dayInMonth) {
						...
						if ($isWorked) {
							...
							$ds = $attMrk[$isWorked->staff_attendance_type_id];
							$bgClr = $attCls[$isWorked->staff_attendance_type_id];
						} else {
							if ($isWeekend == 'Sun') {
								$ds = "Sun";
								$bgClr = 'snDy';
			...
			if ((($post['month'] != '') && ($post['year'] != '')) && ($post['month'] != 'all')) {
				...
				for ($i = 1; $i <= 31; $i++): if ($i <= $dayInMonth) {
				...
				$dayInMonth = date("t", strtotime(date('Y-m')));
				$lstTdShw = '<th>S.N.</th><th>EMP. ID</th><th>EMPLOYEE NAME</th>';
				for ($i = 1; $i <= $dayInMonth; $i++): $lstTdShw .= '<th><div class="thCl">' . $i . '</div></th>';
				endfor;
	...
	public function by_user($getActn = NULL)
	{
		if ($getActn == 'viewList') {
			$post_data = $this->input->post();
			if ($post_data['empSrchdID']) {
				$post_data['empId'] = $post_data['empSrchdID'];
			} else {
				$post_data['empId'] = $this->logId;
			}
			...
			//echo $this->db->last_query().'<br>';
			//SELECT * FROM `staff_attendance` WHERE `employee_id` = '21' AND `attendance_date` >= '2025-04-01' AND `attendance_date` <= '2025-04-30' ORDER BY `attendance_date` DESC LIMIT 30 
			//	exit;
			$i = $post_data['start'] + 1;
			$return['data'] = array();
			foreach ($record as $row) {
				...
				$atStatus = array(
					'1' => '<span class="badge bgPrsnt miBgs">Present</span>',
					'2' => '<span class="badge bgLat miBgs">Late</span>',
					'3' => '<span class="badge bgAbsnt miBgs">Absent</span>',
					'4' => '<span class="badge bgHoliDy miBgs">Holiday</span>',
					'5' => '<span class="badge bgHfDy miBgs">Half Day</span>',
					'6' => '<span class="badge bgLvDy miBgs">Leave</span>'
				);


				//   $logOt=$row->log_out_time?$row->log_out_time.'<br>':'N/A<br>';

				// echo 'Log In :: '.$row->log_in_time?$row->log_in_time.'<br>':'N/A  Logout :: '.$logOt;
				/*	 echo date('d-m-Y',strtotime($row->attendance_date)).' Login Time :: '.($row->log_in_time?$row->log_in_time:($row->log_out_time?date('H:i:s',strtotime($row->shift_start)):'')).' /Logout Time ::'.($row->log_out_time?$row->log_out_time:($row->log_in_time?date('H:i:s',strtotime($row->shift_end)):'')).' <br>';*/
				// print_r($row);echo '<br>';
				$nLoginTime = ($row->log_in_time ? $row->log_in_time : ($row->log_out_time ? date('H:i:s', strtotime($row->shift_start)) : ''));
				$nLogoutTime = ($row->log_out_time ? $row->log_out_time : ($row->log_in_time ? date('H:i:s', strtotime($row->shift_end)) : ''));



				if ($nLogoutTime) {

					$logOut_time = strtotime($nLoginTime);
					$logIn_time = strtotime($nLogoutTime);
					$getTotalMinute = round(abs($logIn_time - $logOut_time) / 60, 2);
					$workingHrs = round(($getTotalMinute / 60), 2);
				...
				$return['data'][] = array(
					...
					'<div class="text-center">' . $atStatus[$row->staff_attendance_type_id] . '</div>',
					$nLoginTime,
					$nLogoutTime,
					$workingHrs,
			...
			if ($empDetail) {
				...
				echo '<li class="miNoEmp"><i class="si si-user"></i> No employee available</li>';
			}
		} else if ($getActn == 'monthwise') {
			$post = $this->input->post();/*$post=array('id'=>'118','attMonth'=>'January','attYear'=>'2025');*/
			sleep(1);
			if ($post['attMonth'] != '' && $post['attYear'] != '') {
				$selectMnthYear = date("Y-m-", strtotime($post['attYear'] . '-' . $post['attMonth']));
				$dayInMonth = date("t", strtotime($post['attYear'] . '-' . $post['attMonth']));
			...
			$where = array(
				'id' => $this->logId,
				'strtDt' => date('Y-m-') . '01',
				'endDt' => date('Y-m-') . $dayInMonth,
				'dayInMonth' => $dayInMonth
			);
			$empAttendance = $this->attendance->getEmployeeAttendance($where);
			$data = array(
				'title' => 'Attendance By User',
				'breadcrums' => 'Attendance By User',
				'target' => 'admin/attendance/by_user/viewList',
				'attMonthwise' => base_url('admin/attendance/by_user/monthwise'),
				'frSearchEmplyee' => base_url('admin/attendance/by_user/findEmployee'),
				'layout' => "admin/attendance/report_byuser.php",
		...
		if (!$isAlreadyExist) {
			...
			if ($findShift) {
				if ((strtotime('- 30 minutes', $findShift->shift_start) < strtotime(date("h:i:s a"))) && (strtotime(date("h:i:s a")) < strtotime($findShift->shift_end))) {
					...
					$mrkPresent = array(
						'employee_id' => $where['empID'],
						'staff_attendance_type_id' => $workingPeriod,
						'attendance_date' => $loginDt,
						'log_in_time' => $loginTime,
						'markIpAddress' => $sysIP,
						'workFrm' => '1',
						'biometric_attendence' => '1',
						'biometric_device_data' => $where['deviceNo'],
						'remark' => 'Biometric Attendance',
						'created_at' => date('Y-m-d')
					);
					//$msg=$mrkPresent;
	...
	public function traceLocation()
	{
		...
		if ($post['dtType'] != 'custom') {
			...
			if ($userDetail) {
				...
				$empInDetails = $userDetail['in_location'] ? $empInDetails : 'Untraceable';
				$empOutDetails = $userDetail['out_location'] ? $empOutDetails : 'Untraceable';
				$inLoc = $userDetail['in_location'] ? $userDetail['in_location'] : 'unTrace';
				$outLoc = $userDetail['out_location'] ? $userDetail['out_location'] : 'unTrace';
				$return = array('actnCls' => 'success', 'empInTime' => $empInDetails, 'inLoc' => $inLoc, 'empOutTime' => $empOutDetails, 'outLoc' => $outLoc);
			} else {
				$return = array('actnCls' => 'error', 'msg' => '<i class="si si-exclamation"></i> Unfortunately, there are no records available.');
			}
		} else {
			...
			if ($customDetail) {
				...
				$empInDetails = $customDetail['in_location'] ? $empInDetails : 'Untraceable';
				$empOutDetails = $customDetail['out_location'] ? $empOutDetails : 'Untraceable';
				$inLoc = $customDetail['in_location'] ? $customDetail['in_location'] : 'unTrace';
				$outLoc = $customDetail['out_location'] ? $customDetail['out_location'] : 'unTrace';
				$return = array('actnCls' => 'success', 'empInTime' => $empInDetails, 'inLoc' => $inLoc, 'empOutTime' => $empOutDetails, 'outLoc' => $outLoc);
			} else {
				$return = array('actnCls' => 'error', 'msg' => '<i class="si si-exclamation"></i> Unfortunately, there are no records available.');
			}
	...
	public function customize($getActn = NULL)
	{
		if ($getActn == 'viewList') {
			...
			foreach ($record as $row) {
				...
				for ($mi = 1; $mi <= $numDay; $mi++):
					if ($mi <= $numDay) {
						$markDate = date('Y-m-d', strtotime($frmDt . ' +' . ($mi - $isCustom) . ' day'));
						$markDateForWeekend = date('Y-m-d', strtotime($frmDt . ' +' . ($mi - $isCustom) . ' day'));
						$isWeekend = date('D', strtotime($markDateForWeekend));
						//echo $markDateForWeekend.'=='.$isWeekend.'<br>';
						...
						if ($isWorked) {
							...
							if ($isWorked->staff_attendance_type_id == '3') {
								$timing = '<span style="color:#F16D75">Absent</span>';
							} else if ($isWorked->staff_attendance_type_id == '4') {
								$timing = '<span style="color:#0073d9">Holiday</span>';
							} else if ($isWorked->staff_attendance_type_id == '6') {
								$timing = '<span style="color:#009EA6">On Leave</span>';
							} else {
								$timing = $log_in_time . $log_out_time;
							}
						} else {
							if ($isWeekend == 'Sun') {
								$timing = "<span style='color:#8c0303'>Sun</span>";
							} else {
								$timing = 'N/A';
							}
						}
		...
		/*	else if($getActn=='setEmployeeFrAttendance')
		{
			...
			$data = array(
				...
				'breadcrums' => 'Attendance Search By Custom',
				'target' => 'admin/attendance/customize/viewList',
				'dayInMonth' => $dayInMonth,
				'attMonthwise' => base_url('admin/attendance/customize/monthwise'),
				'frExcelExport' => 'excelExport===admin/report/export',
				'frPdfExport' => 'pdfExport===admin/report/export',
