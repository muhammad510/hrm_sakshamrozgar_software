...
class Branch extends CI_Controller
{
	...
	public function index($actn = NULL)
	{
		if ($actn == 'view') {
			...
			$getLast = $this->common->get_last('branch_manage', 'code');
			if ($getLast) {
				$lastID = str_replace("BR", "", $getLast['code']);
				$lastID += 1;
				if ($lastID < 9) {
					$newBranchID = 'BR000' . $lastID;
				} else if ($lastID < 99) {
					$newBranchID = 'BR00' . $lastID;
				} else if ($lastID < 999) {
					$newBranchID = 'BR0' . $lastID;
				} else {
					$newBranchID = 'BR' . $lastID;
				}
			} else {
				$newBranchID = 'BR001';
			}
	...
	public function manage()
	{
		...
		if ($post['oprType'] == 'addNew') {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$found = $this->db_model->select_multi('*', 'branch_manage', $con);

				if ($found > 0) {
					$data = array(
						'addClas' => 'tWarning',
						'msg' => array('Branch ID already exist. Please refresh it.'),
					...
					if ($this->common->save_data('branch_manage', $branchArr)) {
						$data = array(
							'addClas' => 'tSuccess',
							'msg' => array('Thank You! you have successfully create new branch.'),
				...
				$msg = array(
					'brNwName' => form_error('brNwName'),
					'contactName' => form_error('contactName'),
					'brLatitude' => form_error('brLatitude'),
					'brLongitude' => form_error('brLongitude'),
					'contactNu' => form_error('contactNu'),
					'mobileNu' => form_error('mobileNu'),
					'emailID' => form_error('emailID'),
					'brAddress' => form_error('brAddress'),
					'get_state' => form_error('get_state'),
			...
			if ($getData) {
				if ($getData->status == '1') {
					$data = array(
						'msg' => '<i class="bx bx-power-off"></i> Are you sure want to deactivate ' . $getData->branch_name . ' of Branch ID #' . $getData->code,
						'action' => 'miStatusChange===software/branch/manage===' . $getData->id
					);
				} else {
					$data = array(
						'msg' => 'Are you sure want to activate ' . $getData->branch_name . ' of Branch ID #' . $getData->code,
						'action' => 'miStatusChange===software/branch/manage===' . $getData->id
					);
				}
			} else {
				$data = array('msg' => '<span class="text-danger">Oops it seems something went wrong</span>');
			}
			...
			if ($getData->status == '1') {
				$change = '2';
				$data = array('msg' => '<span class="text-success"><i class="bx bx-power-off"></i> You have successfully deactivate ' . $getData->branch_name . ' of Branch ID #' . $getData->code . '</span>', 'btnTxt' => 'Deactive', 'btnAdCls' => 'bg-danger bgDng', 'btnRmvCls' => 'bg-success miLvs');
			} else {
				$change = '1';
				$data = array('msg' => '<span class="text-success">You have successfully activate ' . $getData->branch_name . ' of Branch ID #' . $getData->code . '</span>', 'btnTxt' => 'Active', 'btnAdCls' => 'bg-success miLvs', 'btnRmvCls' => 'bg-danger bgDng');
			}
			...
			if ($result) {
				...
				$data = array('msg' => '<span class="text-danger">Oops it seems something went wrong while updating.</span>');
			}
			...
			if ($getData) {
				...
				if ($getParameter) {
					...
					foreach ($getParameter as $list) {
						if ($list->parent_id == '729') {
							if ($list->id == $getData->state) {
								$getStat .= '<option value="' . $list->id . '" selected="selected">' . $list->state_cities . '</option>';
							} else {
								$getStat .= '<option value="' . $list->id . '">' . $list->state_cities . '</option>';
							}
						} else {
							if ($list->id == $getData->district) {
								$getDist .= '<option value="' . $list->id . '" selected="selected">' . $list->state_cities . '</option>';
							} else {
								$getDist .= '<option value="' . $list->id . '">' . $list->state_cities . '</option>';
							}
						}
					}
				} else {
				...
				$getData->getDistrict = $getDist;
				$getData->getState = $getStat;
			} else {
				$data = array('addClas' => 'tDanger', 'msg' => array('Oops it seems something went wrong while updating.'), 'icon' => '<i class="pe-7s-sun bx-spin"></i>');
			}
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($result) {
					$data = array(
						'addClas' => 'tSuccess',
						'msg' => array('Thank You! you have successfully updated branch details.'),
						'icon' => '<i class="ti-check-box"></i>'
					);
				} else {
					$data = array(
				...
				$msg = array(
					...
					'emailID' => form_error('emailID'),
					'brAddress' => form_error('brAddress'),
					'getDepIdFr' => form_error('getDepIdFr'),
					'getDesigIdFr' => form_error('getDesigIdFr'),
					'get_state' => form_error('get_state'),
					'district' => form_error('district'),
					'zipCode' => form_error('zipCode')
				);
			...
			$data = array(
				'addClas' => 'tSuccess',
				'fDataResource' => '<option>Trsting mode</option>'
			);
		} else {
			$data = array(
				'addClas' => 'tDanger',
		...
		if ($action == 'department') {
			...
			if ($getDpId) {
				...
				if ($getDesigList) {
					...
					$desigArr = '<li>There is no data available here</li>';
			...
			sleep(1.5);
			if (strlen($setDepartment) > 20) {
				$department = substr($setDepartment, 0, 25) . '...';
			} else {
				$department = $setDepartment;
			}
			if (strlen($setDesignation) > 20) {
				$setDesign = substr($setDesignation, 0, 25) . '...';
			} else {
				$setDesign = $setDesignation;
			}
			...
			if ($getDpId) {
				...
				if ($getDesigList->designations) {
					if (strlen($getDesigList->designations) > 30) {
						$titleDesign = substr($getDesigList->designations, 0, 30) . '...';
					} else {
						$titleDesign = $getDesigList->designations;
					}
					$resultAction = 'success';
				} else {
					$titleDesign = 'Select Designation';
					$resultAction = 'failed';
				}
			} else {
				$titleDesign = 'Select Department';
				$resultAction = 'failed';
			}
	...
	private function depart_designation($where)
	{
		$depArr = array();
		if ($where['depId']) {
			$getAlDepart = $this->common->getDataList('department', 'status', '1');
			$deprt = '';
			if ($getAlDepart) {
				$getSelDep = explode(',', $where['depId']);
				$setDep = '';
				$dpCnt = 0;
				foreach ($getAlDepart as $dep) {
					...
					$matchDepart = (in_array($dep->id, $getSelDep)) ? '&#10003;' : '';
					$mrkDepart = (in_array($dep->id, $getSelDep)) ? 'mrkAdd' : '';
					if (in_array($dep->id, $getSelDep)) {
						$dpCnt++;
						$setDep .= ($dpCnt == '1') ? $dep->department_name : ',' . $dep->department_name;
					}
			...
			$getDesigList = $this->branch->getDesignationListByDepartment($whereDep);
			$design = '';
			if ($getDesigList) {
				$getSelDesig = explode(',', $where['desigId']);
				$setDesig = '';
				$desigCnt = 0;
				$desigUrl = base_url('software/branch/administration/designation');
				foreach ($getDesigList as $desigList) {
					$matchDesig = (in_array($desigList->id, $getSelDesig)) ? '&#10003;' : '';
					$mrkDesig = (in_array($desigList->id, $getSelDesig)) ? 'mrkAdd' : '';
					if (in_array($desigList->id, $getSelDesig)) {
						$desigCnt++;
						$setDesig .= ($desigCnt == '1') ? $desigList->designation_name : ',' . $desigList->designation_name;
					}
					$desigArr .= '<li id="desig--' . $desigList->id . '" data-id="' . $desigUrl . '" class="depCollect">
			...
			$depArr = array(
				'depShow' => (strlen($setDep) > 20) ? (substr($setDep, 0, 25) . '...') : $setDep,
				'departList' => $deprt,
				'desigShow' => (strlen($setDesig) > 20) ? (substr($setDesig, 0, 25) . '...') : $setDesig,
				'desigList' => $desigArr
			);
		}
		return $depArr;
