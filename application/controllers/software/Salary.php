		...
		if ($actn == 'view') {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($post['grsSalAmt'] == 0) {
					...
					if (number_format($isDistribution, 2) > '100.00') {
						$data = array('addClas' => 'tst_warning', 'msg' => array('Salary distribution limit exceeded !'), 'icon' => '<i class="fe fe-settings bx-spin"></i>');
					} else if (number_format($isDistribution, 2) < '100.00') {
						$data = array('addClas' => 'tst_warning', 'msg' => array('Salary distribution less than 100% !'), 'icon' => '<i class="fe fe-settings bx-spin"></i>');
					} else {
						...
						if (!$isExistSalary) {
							...
							$data = array(
								'addClas' => 'tst_warning',
								'msg' => array('Oops it seems salary details is already exist!'),
								'icon' => '<i class="fe fe-settings bx-spin"></i>'
							);
						}
					}
				...
				$msg = array(
					'brOffice' => form_error('brOffice'),
					'designation' => form_error('designation'),
					'grsSalAmt' => form_error('grsSalAmt'),
			...
			if ($getDetails) {
				...
				$return = array(
					'addClas' => 'tst_danger',
					'msg' => array('Oops there is an error while fetching details!'),
					'icon' => '<i class="fe fe-settings bx-spin"></i>'
				);
			}
			echo json_encode($return);
			...
			if ($getDetails['designation']) {
				...
				if ($department) {
					...
					$result = '<option value="">Choose Designation</option>';
				}
			} else {
				$result = '<option value="">Choose Designation</option>';
			}
			sleep(.5);
			print_r($result);
		} else if ($actn == 'findDepartmentDetails') {
			$post = $this->input->post();
			$getDetails = $this->common->get_data('branch_manage', array('id' => $post['brID']), 'department');
			if ($getDetails['department']) {
				...
				if ($department) {
					...
					foreach ($department as $dList) {
						$desigDet = $this->common->get_data('department', array('id' => $dList), 'department_name');
						if ($desigDet) {
							$dpList .= '<option value="' . $dList . '">' . $desigDet['department_name'] . '</option>';
						}
					}
					$result = $dpList;
				} else {
			...
			$data['getDepartment'] = base_url('software/salary/index/findDepartmentDetails');
	...
	public function manage()
	{
		...
		if ($post['oprType'] == 'miStatusView') {
			...
			if ($getData) {
				...
				$msg = array(
					'msg' => '<span class="text-danger">Oops it seems something went wrong</span>'
				);
			}
			...
			if ($getData->status == '1') {
				$change = '2';
				$msg = array(
					'msg' => '<span class="text-success"><i class="si si-power"></i> You have successfully deactivate of Salary ID #' . $getData->salary_code . '</span>',
					'btnTxt' => 'Deactive',
					'btnAdCls' => 'bg-danger ',
					'btnRmvCls' => 'bg-success miLvs'
				);
			} else {
				$change = '1';
				$msg = array(
					'msg' => '<span class="text-success">You have successfully activate of Salary ID #' . $getData->salary_code . '</span>',
					'btnTxt' => 'Active',
					'btnAdCls' => 'bg-success miLvs',
					'btnRmvCls' => 'bg-danger'
				);
			}
			$changeArr = array(
			...
			if ($result) {
				...
				$msg = array(
					'msg' => '<span class="text-danger">Oops it seems something went wrong while updating.</span>'
				);
			}
			echo json_encode($msg);
