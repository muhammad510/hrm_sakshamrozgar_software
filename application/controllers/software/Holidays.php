...
class Holidays extends CI_Controller
{
    ...
    public function update_holidays()
    {
        ...
        $data['update_holiday'] = $this->common->get_data('holidays', array('id' => $update['id']), '*');
    ...
    public function update_holidays_data()
    {
        ...
        $this->form_validation->set_rules('holiday_name', 'Holiday Name', 'required');
        $this->form_validation->set_rules('from_date', 'From Date', 'required');
        $this->form_validation->set_rules('to_date', 'From Date', 'required');
    ...
    public function manage()
    {
        ...
        if ($post['oprType'] == 'miStatusView') {
            ...
            if ($getData->status == '1') {
                $change = '2';
                $msg = array('msg' => '<span class="text-success"><i class="si si-power"></i> You have successfully deactivate ' . $getData->holiday_name . ' of ID #' . $getData->id . '</span>', 'btnTxt' => 'Deactive', 'btnAdCls' => 'bg-danger ', 'btnRmvCls' => 'bg-success miLvs');
            } else {
                $change = '1';
                $msg = array('msg' => '<span class="text-success">You have successfully activate ' . $getData->holiday_name . ' of ID #' . $getData->id . '</span>', 'btnTxt' => 'Active', 'btnAdCls' => 'bg-success miLvs', 'btnRmvCls' => 'bg-danger');
            }
