...
class Dashboard extends CI_Controller {
	public function __construct()
	{
		...
		$this->load->model(array('admin/Attendance_model'=>'attendance','employee/Dashboard_model'=>'dashboard'));
...
/**************************************************************************/	
	 ...
	 /*-------------------------------Holiday Manage start----------------------------*/
		  ...
		  $daysInMonth=array('01'=>'31','02'=>'28','03'=>'31','04'=>'30','05'=>'31','06'=>'30','07'=>'31','08'=>'31','09'=>'30','10'=>'31','11'=>'30','12'=>'31');
		  $frmDate=date('Y-m').'-01';$endDate=date('Y-m').'-'.$daysInMonth[date('m')];
		  $findHolidays=$this->attendance->getHolidaysListInMonth($frmDate,$endDate);
		  $isHolidaysSet=$this->attendance->isHolidaysListInMonth($frmDate,$endDate);
				...
				foreach($findHolidays as $hList)
				{
					...
					if($getEmpList)
					{
						foreach($getEmpList as $emp)
						{
							...
							if(!$isAlreadyExist)
							{
								$createHoliday=array('employee_id'=>$emp->id,'staff_attendance_type_id'=>'4','attendance_date'=>$hList->holiday_date,
												     'holiday_id'=>$hList->id,'log_in_time'=>$emp->shift_start,'log_out_time'=>$emp->shift_end,'created_at'=>date('Y-m-d'));
	...
	/*-------------------------------Holiday Manage End----------------------------*/
		...
		$data=array('tEmpPresent'=>$tEmpPresent,'tEmpLate'=>$tEmpLate,'tEmpAbsent'=>$tEmpAbsent,'tEmpHfDy'=>$tEmpHfDy,'tEmpLeave'=>$tEmpLeave,'totalEmp'=>$totalEmp,
					'prsntPr'=>round((($tEmpPresent*100)/$totalEmp),2),'latePr'=>round((($tEmpLate*100)/$totalEmp),2),'absntPr'=>round((($tEmpAbsent*100)/$totalEmp),2),
					'halfDyPr'=>round((($tEmpHfDy*100)/$totalEmp),2),'leavePr'=>round((($tEmpLeave*100)/$totalEmp),2),'totalSalary'=>$totalSalary,
					'resigned'=>$resigned,'isNewEmp'=>$isNewEmp,'employee'=>$employee,
					'thisMonthSalary'=>$thisMonthSalary,'recentJoinee'=>$recentJoinee,'recentTurnover'=>$recentTurnover,'todayAttendance'=>$todayAttendance,
					'thisMonthHoliday'=>$thisMonthHoliday,'empUpComingBdy'=>$empUpComingBdy,'title'=>'Dashboard Panel','breadcrums'=>'Dashboard Panel',
					'all_row_punch'=>$temp_all_row_punch,'expenseOverview'=>$expenseOverview,'punchOverview'=>$punchOverview,'workingAvg'=>$workingAvg,'dailyWorkingGraph'=>$dailyWorkingGraph
					);
