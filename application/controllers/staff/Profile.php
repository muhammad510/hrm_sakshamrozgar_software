...
class Profile extends CI_Controller
{
	...
	public function bank_update($getId)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			$post = $this->input->post();
			$proDetails = array(
				'holder_name' => $post['acHldrname'],
				'bank_name' => $post['bnk_name'],
				'acc_typ' => $post['acType'],
				'bank_account_no' => $post['bnkAcNumber'],
				'bank_branch' => $post['brnchName'],
			...
			$msg = array(
				'acHldrname'      => form_error('acHldrname'),
				'bnk_name'        => form_error('bnk_name'),
				'acType'          => form_error('acType'),
				'bnkAcNumber'     => form_error('bnkAcNumber'),
				'brnchName'       => form_error('brnchName'),
	...
	public function details($where)
	{
		...
		if ($where->action == 'attendanceSummary') {
			...
			foreach ($record as $row) {
				...
				if ($row->log_out_time) {
					...
					$workingHrs = '<span class="wrKng">Still working...</span>';
					$logOutTiming = NULL;
				}
			...
			foreach ($record as $row) {
				if ($row->leave_mode == '1') {
					$endDt = date('d-m-Y', strtotime($row->from_date));
				} else {
					$endDt = date('d-m-Y', strtotime($row->end_date));
				}
				if ($row->total_leave == '1') {
					$leaveDate = '1 Day';
				} else {
					$leaveDate = $row->total_leave . ' Days';
				}
				if ($row->created_date == date('Y-m-d')) {
					$status = '<span class="badge bg-primary miWdth"> New</span>';
				} else if ($row->status == '4') {
					$status = '<span class="badge bg-warning miWdth">Pending</span>';
				} else if ($row->status == '1') {
					$status = '<span class="badge bg-success miWdth">Approved</span>';
				} else if ($row->status == '2') {
					$status = '<span class="badge bg-danger miWdth">Rejected</span>';
				} else if ($row->status == '3') {
					$status = '<span class="badge bg-secondary miWdth">Hold</span>';
				}
				$delBtn = 'Hello';
				...
				if ($row->status == '1') {
					...
					$trashBtn = '<button type="button" class="btn btn-outline-danger btn-sm miAction" data-id="delOperation==' . $delBtn . '"><i class="fe fe-trash-2"></i></button>';
				}
		...
		<button type="button" class="btn btn-outline-primary btn-sm miAction" data-id="watchLeaveDetails===/employee/leave/view===' . $row->id . '"><i class="fe fe-eye"></i></button> 
				...
				if (!$isDocument) {
					...
					if ($this->upload->do_upload('empDocFileAdd')) {
						...
						$createNewDocument = array(
							'emp_id' => $where->id,
							'doc_type' => $post['docuType'],
							'doc_image' => 'uploads/employee/documents/' . $image_filename,
							'created_date' => date('Y-m-d H:i:s a'),
							'created_id' => $this->logId
						);
						$srCnt = $this->staff->getRowCount($where->id);
						$nwRoCnt = count(explode('----', $srCnt->id)) + 1;
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$lvDet = count($getLvArr);
				$newLvDet = '';
				if ($lvDet > count($post['current_leave'])) {
					for ($x = 0; $x < $lvDet; $x++) {
						if ($x < count($post['current_leave'])) {
							$preLvArr = explode('==', $getLvArr[$x]);
							if ($preLvArr[0] == $post['current_leave'][$x]) {
								$newLvDet = $newLvDet ? ($newLvDet .= ',' . trim($getLvArr[$x])) : trim($getLvArr[$x]);
							} else {
					...
					for ($x = 0; $x < count($post['current_leave']); $x++) {
						...
						if (in_array($preLvArr[0], $post['current_leave'])) {
							$newLvDet = $newLvDet ? ($newLvDet .= ',' . trim($getLvArr[$x])) : trim($getLvArr[$x]);
							unset($post['current_leave'][$x]);
						}
					}
					if ($post['current_leave']) {
						$getCurrentLv = $this->staff->getCreditLeaveDataByArr($post['current_leave']);
						$nwOptedLeave = str_replace(" ", "", $getCurrentLv->approvLeave);
					}
				...
				if ($post['salID'] != '9999') {
					$getSalAmt = $this->common->getRowData('salary_master', 'id', $post['salID']);
				} else {
				...
				if ($updateResult) {
					...
					$data = array(
						...
						'joinDate' => $joinDate,
						'branch_name' => ($emp->branch_name ? $emp->branch_name : 'Super Admin'),
						'department_name' => $emp->department_name,
						'designation_name' => $emp->designation_name,
						'creditLv' => $getNewApprLeave->aprLeave,
						'noticeDt' => $notify_period,
						'notiFyRmrks' => $notiFyRmrks
					);
				...
				$msg = array(
					...
					'department_nm' => form_error('department_nm'),
	...
	public function trashDetails()
	{
		...
		if ($post['action'] == 'cnfDeleteDoc') {
			...
			if ($isExistDocument) {
				...
				if ($isDeleteDoc) {
					if ($isExistDocument->doc_image != 'uploads/employee/documents/no_doc.png') {
						unlink($isExistDocument->doc_image);
					}
					...
					$data = array('addClas' => 'danger', 'msg' => 'Oops it seems something went wrong please refresh it');
				}
			} else {
				$data = array('addClas' => 'danger', 'msg' => 'Oops it seems there is no document available here');
			}
		}
		//print_r($data);
		echo json_encode($data);
			...
			if ($post['grsSalAmt'] == 0) {
				...
				if (!$isExistSalary) {
					...
					if ($isCurrentSal) {
						...
						$data = array('addClas' => 'tst_warning', 'msg' => array('Oops there is an error while creating!'), 'icon' => '<i class="fe fe-settings bx-spin"></i>');
					}
				} else {
					$changeArr = array(
						'sal_mstr_id' => $isSalaryId['id'],
					...
					if ($result) {
						...
						$data = array('addClas' => 'tst_warning', 'msg' => array('Oops there is an error while updating!'), 'icon' => '<i class="fe fe-settings bx-spin"></i>');
					}
				}
			}
		} else {
