...
class Site extends CI_Controller {
	...
	public function forgetPassword()
	{
		...
		else if(filter_var($post,FILTER_VALIDATE_EMAIL)===false){$msg=array('Please input valid registered email id');
		...
		else
		{
			...
			if($isCheckUser)
			{
				...
				if(!$isCheckSentRequest)
				{
					...
					$data=array('adClass'=>'tst_success','msg'=>$msg,'icon'=>'<i class="ti-check-box"></i>',
		...
		if($post)
		{
			...
			if($isCheckUser)
			{
				...
				if($isCurrent < $isCheckLinkExpired)
				{
					...
					{
						$msg=array('Oops it seems your reset password link is expired.');$data=array('adClass'=>'tst_warning','msg'=>$msg,'icon'=>'<i class="fe fe-settings bx-spin"></i>','inptErr'=>'parsley-warning');
						}sleep(3);
			echo json_encode($data);
			}
			else
