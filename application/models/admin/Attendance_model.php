...
public function get_attendance_last($where){return $this->db->select('id,log_in_time,staff_attendance_type_id,log_out_time')->where($where)->get('staff_attendance')->row();}
// ===========================Attendance Report List Start @mit Singh============================
	...
	public function employeeAttendance_query($where=false,$cShift=NULL)
	{
		...
		if($this->logCat=='5'){$this->db->where('s.branch_id',$this->logBr)->where('s.user_type','4');}
...
// ===========================Attendance Report List End @mit Singh============================	
	public function empWiseAttendance_query($where=false)
	{
		...
		if(!empty($where['search']['value']))
		{
			if($i===0){$this->db->group_start();$this->db->like($item, $where['search']['value']);} 
		  ...
		  if(empty($where['empAttDate']) && empty($where['attMonth'])&& empty($where['attYear']))
            {
                $curentMonthYear=date('Y-m');$dayInMonth=date("t",strtotime($curentMonthYear));
				$this->db->where('attendance_date >=',$curentMonthYear.'-01')->where('attendance_date <=',$curentMonthYear.'-'.$dayInMonth);
            }
		if(!empty($where['empAttDate'])){ $this->db->where('attendance_date',date('Y-m-d',strtotime(str_replace("/","-",$where['empAttDate']))));}
	...
	/*	
		...
		else{$this->db->order_by('attendance_date','desc');}
	}

	public function empWiseAttendanceList($where=false)
	{
		$this->empWiseAttendance_query($where);if($where['length']!=-1){$this->db->limit($where['length'],$where['start']);}return $this->db->get()->result();
		}

	public function empWiseAttendance_count($where=false)
	{
		$this->empWiseAttendance_query($where);return $this->db->get()->num_rows();
	}
...
public function getEmployeeAttendance($where)
{
	...
	$present=$isPresent->p;$late=$isLate->l;$hfDy=$isHfDy->hf;	$absent=$isAbsent->ab;	
	$byCmpnyLv=$byCompanyLeave->byCmpnyLv;$tWorkingDay=$where['dayInMonth']-($byCmpnyLv+$num_sundays);		   
