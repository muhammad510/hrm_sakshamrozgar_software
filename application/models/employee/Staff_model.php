...
*/
	...
	{
	  $getDetails=array();$lastDyInMnth = date("Y-m-t");
	  ...
	  if($nHolidy)
	  {
		 ...
		 if($nHolidy->feast){$wrkngDy=(int)$getTdays[date('m')];$getDetails['wrkngDy']=$wrkngDy-$nHolidy->feast;}else{ $getDetails['wrkngDy']='0';}
		 }
		...
		if($nPresent){$getDetails['present']=$nPresent->p;}else{$getDetails['present']='0';} 
		...
		if($nAbsent){$getDetails['absent']=$nAbsent->a;}else{$getDetails['absent']='0';} 	
		...
		if($nHalfDy){$getDetails['halfDy']=$nHalfDy->hfdy;}else{$getDetails['halfDy']='0';} 
		...
		if($nLateDy){$getDetails['lateDy']=$nLateDy->ltdy;}else{$getDetails['lateDy']='0';} 
		return $getDetails;
	...
	{
		return $this->db->select('branch_name,department_name,designation_name,s.*')->from('staff as s')->where('s.id',$id)->join('branch_manage as br', 's.branch_id=br.id','left')->join('department as dep', 'dep.id=s.department','left')->join('designation as desig', 'desig.id=s.designation','left')->get()->row();	
...
/*            if(!empty($where['leaveID'])){ $this->db->where('leaveID',$where['leaveID']);}
        ...
        public function feedback_list($where = false,$empID=NULL){$this->feedback_query($where,$empID);if ($where['length'] != -1) {$this->db->limit($where['length'], $where['start']);}return $this->db->get()->result();}
        public function feedback_count($empID=NULL){$this->feedback_query($where = false,$empID);return $this->db->get()->num_rows(); }
        public function feedback_filter_count($where = false,$empID=NULL){$this->feedback_query($where,$empID);return $this->db->get()->num_rows();}			
