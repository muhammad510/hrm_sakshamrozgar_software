...
$this->db->select('empLv.id,empLv.emp_id,empLv.leave_mode,empLv.leave_type,s.employee_id,s.name,leave_name,empLv.from_date,empLv.end_date,empLv.total_leave,empLv.reason,empLv.status,empLv.created_date')->from('employee_leave as empLv')->join('leave_manage as lvM', 'lvM.id=empLv.leave_type','left')->join('staff as s', 's.id=empLv.emp_id','left');
        ...
        {
			$this->applied_leave($where,$apply); if($where['length'] != -1){$this->db->limit($where['length'], $where['start']);}return $this->db->get()->result();
        }

        public function applied_leave_count($apply=NULL){$this->applied_leave($where=false,$apply);return $this->db->get()->num_rows();}
        public function applied_leave_filter_count($where=false,$apply=NULL){$this->applied_leave($where,$apply);return $this->db->get()->num_rows();}
/*-----------------------------leave Applied End-------------------------------*/
...
$this->db->select('empLv.id,empLv.emp_id,empLv.leave_mode,empLv.leave_type,s.employee_id,s.name,leave_name,empLv.from_date,empLv.end_date,empLv.total_leave,empLv.reason,empLv.status,empLv.created_date')->from('employee_leave as empLv')->join('leave_manage as lvM', 'lvM.id=empLv.leave_type','left')->join('staff as s', 's.id=empLv.emp_id','left');
		if($type){$this->db->where('empLv.status',$type);}
	...
	{
		$this->is_leave($where,$type); if($where['length'] != -1){$this->db->limit($where['length'], $where['start']);}return $this->db->get()->result();
	}
	public function leave_counter($type=NULL){$this->is_leave($where=false,$type);return $this->db->get()->num_rows();}
	public function leave_filter_counter($where=false,$type=NULL){$this->is_leave($where,$type);return $this->db->get()->num_rows();}
