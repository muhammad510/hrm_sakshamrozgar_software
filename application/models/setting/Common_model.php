...
class Common_model extends CI_Model
{
	...
	{
		$curTime = date('H:i:s');
		if (($curTime > '02:00:00') && ($curTime < '10:00:00')) {
			$return = array('curntShift' => '17', 'offShift' => '18');
		} else if (($curTime > '10:00:00') && ($curTime < '18:00:00')) {
			$return = array('curntShift' => '18', 'offShift' => '19');
		} else if (($curTime > '18:00:00') && ($curTime < '23:59:59')) {
			$return = array('curntShift' => '19', 'offShift' => '18');
		} else {
			$return = array('curntShift' => '17', 'offShift' => '18');
		}
		return $return;
	...
	{
		$return = $this->db->select('*')->from('company_income')->where('id >', $where['id'])->get()->result();
		$result = false;
		if ($return) {
			$cnt = 0;
	...
	public function createMonhtlyReport($where)
	{
		...
		$this
			->db
			->from('staff as s')
			->where('sf.employee_id', $empId->id)
			->where('sf.attendance_date >=', $where['frmDate'])
			->where('sf.attendance_date <=', $where['lastDate'])
			->join('staff_attendance as sf', 'sf.employee_id=s.id', 'left')
			->join('designation as d', 'd.id=s.designation', 'left')
			->join('department as dept', 'dept.id=s.department', 'left')
			->join('branch_manage as br', 'br.id=s.branch_id', 'left')
			->join('shift_manage as sft', 'sft.id=s.shift', 'left')
			->group_by('sf.employee_id')
			->order_by('');
		$result = $this->db->get();
		return $result->result();
				...
				if ($where['srNo'] == '2') {
					...
					if ($numDay >= $i) {
						...
						if (($getLogin->log_out_time != NULL) && ($getLogin->log_in_time != NULL)) {
							if ($getLogin->log_in_time != NULL) {
								$getTotalMinute = round(abs(strtotime($getLogin->log_in_time) - strtotime($getLogin->log_out_time)) / 60, 2);
							} else {
								$getTotalMinute = round(abs(strtotime($getLogin->log_in_time) - strtotime($where['shift_end'])) / 60, 2);
							}
						} else {
							if ($getLogin->log_in_time != NULL) {
								$getTotalMinute = round(abs(strtotime($getLogin->log_in_time) - strtotime($where['shift_end'])) / 60, 2);
